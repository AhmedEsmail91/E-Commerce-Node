class ApiFeatures{
    ...
    filteration(){
        ...
        filterObj=JSON.parse(JSON.stringify(filterObj).replace(/\b(gte|gt|lte|lt)\b/g,match=>`$${match}`));3
    ...
    search(){
        if(this.seachQuery.keyword){
            this.mongooseQuery.find({$or:[
                {name:{$regex:this.seachQuery.keyword,$options:"i"}},
                {description:{$regex:this.seachQuery.keyword,$options:"i"}}
            ]});
        }
        return this
    }
